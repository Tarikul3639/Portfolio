...
const Navbar = ({ title = "Tarikul", menuItems = ["HOME", "ABOUT", "PROJECTS", "CONTACT"] }) => {
    ...
    return (
        <nav className={`${isOpen ? "bg-background/95 dark:bg-background-dark/95" : " bg-transparent backdrop-blur-xs"} w-full min-w-[250px] text-white px-4 py-0 shadow-xl fixed top-0 left-0 z-50`}>
            <div className="container mx-auto flex items-center justify-between">
                ...
                <ul className={`Menu flex flex-col items-center text-left md:flex-row md:space-x-10 space-y-2 md:space-y-1 pb-6 md:p-0 absolute md:static md:bg-transparent left-0 w-full md:w-auto transition-all duration-300 ease-in-out 
                    ${isOpen ? "top-12 opacity-100 bg-background/95 dark:bg-background-dark/95 shadow-lg md:shadow-none" : "-top-60 opacity-100 h-auto"}`}>
