...
const Projects = () => {
  ...
  return (
    <section id="projects" ref={containerRef} className="relative py-16 lg:py-24 bg-white dark:bg-[#020202] text-neutral-900 dark:text-white overflow-hidden">

      <div className="container relative z-10 mx-auto px-2 md:px-6 max-w-7xl">

        <div ref={contentRef} className="relative pl-3 md:pl-3 lg:pl-6 xl:pl-12 space-y-16 lg:space-y-24">
          ...
          {timelineData.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                <div className="absolute -left-[18px] md:-left-[54px] top-1/2 -translate-y-1/2 z-20">
                  <motion.div
                    initial={{ scale: 0, opacity: 0, y: -100 }}
                    whileInView={{ scale: [0, 1.5, 1], opacity: 1, y: 0 }}
                ...
                >
                  <SpotlightCard className="md:p-8 lg:p-14 border border-neutral-200 dark:border-white/10 bg-white hover:bg-primary/5 dark:bg-[#080808]/60 backdrop-blur-3xl rounded-2xl lg:rounded-[2rem] relative overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.04)] dark:shadow-none transition-all duration-500 hover:border-primary/50 hover:shadow-[0_30px_60px_rgba(0,238,255,0.1)] hover:dark:shadow-[0_30px_60px_rgba(0,238,255,0.2)]">

                    {/* Watermark Text */}
                    <div className={`absolute -bottom-5 -right-0 lg:-bottom-16 lg:-right-10 ${isEven ? '-left-10' : '-right-10'} text-[4rem] md:text-[8rem] lg:text-[14rem] font-black text-neutral-200 dark:text-white/[0.02] italic pointer-events-none select-none uppercase transition-opacity group-hover:opacity-5`}>
                      {item.bgText}
                    </div>

                    <div className={`relative z-10 flex flex-col ${isEven ? 'lg:flex-row-reverse' : 'lg:flex-row'} items-center gap-12 lg:gap-16`}>

                      {/* VISUAL MODULE */}
                      <div className="w-full lg:w-[45%] relative group/imgContainer">

                        {/* --- DYNAMIC GLASSY PROGRESS MODULE --- */}
                        <div className="absolute -right-4 top-1/2 -translate-y-1/2 z-40 hidden md:flex flex-col items-center gap-4 bg-black/2 dark:bg-white/5 backdrop-blur-2xl border border-neutral-200/30 dark:border-white/20 p-4 rounded-2xl lg:rounded-[2rem] shadow-[0_10px_30px_rgba(0,0,0,0.1)] dark:shadow-[0_10px_30px_rgba(0,0,0,0.5)] transition-transform group-hover:scale-102 group-hover:border-primary/50">
                        ...
                        <div className="relative aspect-[16/7] md:aspect-[16/8] lg:aspect-[16/16] xl:aspect-[16/10] rounded-2xl lg:rounded-[2rem] overflow-hidden border border-neutral-200/20 dark:border-white/10 shadow-2xl">
                          <img
                            src={item.image}
                      ...
                      {/* CONTENT MODULE */}
                      <div className={`flex-1 space-y-8 ${isEven ? 'text-center lg:text-right' : 'text-center lg:text-left'}`}>
                        <div className="space-y-4">
                          <div className={`flex items-center gap-3 ${isEven ? 'justify-center lg:justify-end' : 'justify-center lg:justify-start'}`}>
                          ...
                          <h3 className="text-3xl md:text-5xl font-black italic tracking-tighter uppercase leading-[0.85] text-neutral-900 dark:text-white transition-colors duration-500 group-hover:text-primary">
                            {item.title}
                          </h3>
                          <p className="text-neutral-600 dark:text-neutral-400 text-base md:text-lg leading-relaxed max-w-xl mx-auto lg:mx-0">
                            {item.description}
                          </p>
                        </div>

                        {/* Tech Stack - Glassy Pills */}
                        <div className={`flex flex-wrap gap-2 ${isEven ? 'justify-center lg:justify-end' : 'justify-center lg:justify-start'}`}>
                          {item.tech.map((t, i) => (
                        ...
                        {/* Actions */}
                        <div className={`flex flex-wrap gap-4 pb-8 lg:pb-0 ${isEven ? 'justify-center lg:justify-end' : 'justify-center lg:justify-start'}`}>
                          <button
                            onClick={() => item.Live_link && window.open(item.Live_link, "_blank")}
                            className={`
    relative py-4 lg:py-5 px-6 lg:px-8 overflow-hidden rounded-2xl
    font-black text-[10px] lg:text-[11px] uppercase tracking-[0.15em]
  ...
  `}
                          ...
                          <button onClick={() => window.open(item.link, "_blank")} disabled={!item.link} className="py-4 lg:py-5 px-6 lg:px-8 rounded-2xl border border-neutral-200/30 dark:border-white/10 bg-neutral-100/80 dark:bg-white/5 text-neutral-900 dark:text-white font-black text-[11px] uppercase tracking-[0.15em] flex items-center gap-3 transition-all hover:bg-neutral-200 hover:scale-105 active:scale-95 dark:hover:bg-white/10">
