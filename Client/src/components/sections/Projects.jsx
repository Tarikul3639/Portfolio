...
const Projects = () => {
  ...
  return (
    <section id="projects" ref={containerRef} className="relative py-32 bg-[#020202] text-white overflow-hidden">
              ...
              >
                ...
                <div className="absolute -left-[54px] md:-left-[101px] top-1/2 -translate-y-1/2 z-20">
                    <motion.div 
                        ...
                        className="h-3 w-3 rounded-full bg-black border border-primary shadow-[0_0_15px_#00eeff]" 
                    />
                </div>
                ...
                >
                  <SpotlightCard className="relative overflow-hidden border border-white/10 bg-white/[0.01] backdrop-blur-3xl rounded-[2.5rem] p-8 md:p-12 transition-all duration-500 group-hover:border-primary/40 group-hover:bg-white/[0.03] group-hover:shadow-[0_20px_60px_-15px_rgba(0,238,255,0.2)]">
                    
                    {/* Watermark Text */}
                    <div className={`absolute -bottom-10 ${isEven ? '-left-10' : '-right-10'} text-[12rem] md:text-[15rem] font-black text-white/[0.02] italic pointer-events-none select-none uppercase transition-opacity group-hover:opacity-5`}>
                      {item.bgText}
                    </div>
                    ...
                    <div className={`relative z-10 flex flex-col ${isEven ? 'xl:flex-row-reverse' : 'xl:flex-row'} items-center gap-12 xl:gap-16`}>
                      ...
                      <div className="w-full xl:w-[45%] relative group/imgContainer">
                        ...
                        <div className="absolute -right-4 top-1/2 -translate-y-1/2 z-40 hidden md:flex flex-col items-center gap-4 bg-white/5 backdrop-blur-2xl border border-white/20 p-4 rounded-3xl shadow-[0_10px_30px_rgba(0,0,0,0.5)] transition-transform group-hover:scale-102 group-hover:border-primary/50">
                            <div className="flex flex-col items-center gap-2">
                              <span className="text-[7px] font-black text-primary uppercase tracking-[0.2em]">Efficiency</span>
                              <div className="h-32 w-2 bg-black/40 rounded-full overflow-hidden relative border border-white/5">
                            ...
                            <div className="text-sm font-black text-white drop-shadow-[0_0_10px_rgba(0,238,255,0.5)]">{item.progress}%</div>
                            <motion.div animate={{ rotate: 360 }} transition={{ duration: 4, repeat: Infinity, ease: "linear" }}>
                                <FontAwesomeIcon icon={faMicrochip} className="text-xs text-primary/80" />
                            </motion.div>
                        </div>
                        ...
                        <div className="relative aspect-[16/10] rounded-[2rem] overflow-hidden border border-white/10 shadow-2xl">
                          <img 
                            ...
                            alt={item.title} 
                          />
                          <div className="absolute inset-0 bg-gradient-to-tr from-black/80 via-transparent to-white/5 opacity-60" />
                        </div>
                      </div>
                      ...
                      <div className={`flex-1 space-y-8 ${isEven ? 'text-center xl:text-right' : 'text-center xl:text-left'}`}>
                        <div className="space-y-4">
                          ...
                          <h3 className="text-5xl md:text-7xl font-black italic tracking-tighter uppercase leading-[0.85] text-white transition-colors duration-500 group-hover:text-primary">
                            {item.title}
                          </h3>
                          <p className="text-neutral-400 text-base md:text-lg leading-relaxed max-w-xl mx-auto xl:mx-0">
                            {item.description}
                          </p>
                        </div>
                        ...
                        <div className={`flex flex-wrap gap-2 ${isEven ? 'justify-center xl:justify-end' : 'justify-center xl:justify-start'}`}>
                          {item.tech.map((t, i) => (
                            <div key={i} className="px-4 py-2 rounded-xl bg-white/[0.03] border border-white/5 flex items-center gap-2.5 transition-all duration-300 hover:border-primary/40 hover:bg-white/[0.08] hover:-translate-y-1">
                              <span className="text-xs grayscale group-hover:grayscale-0 transition-all">{t.icon}</span>
                              <span className="text-[10px] font-bold uppercase tracking-tight text-neutral-400 group-hover:text-white transition-colors">{t.name}</span>
                            </div>
                          ))}
                        </div>
                        ...
                        <div className={`flex flex-wrap gap-4 pt-4 ${isEven ? 'justify-center xl:justify-end' : 'justify-center xl:justify-start'}`}>
                          ...
                          <button onClick={() => window.open(item.link, "_blank")} className="h-14 px-8 rounded-2xl border border-white/10 bg-white/5 text-white font-black text-[11px] uppercase tracking-[0.15em] flex items-center gap-3 transition-all hover:bg-white hover:text-black hover:scale-105 active:scale-95">
